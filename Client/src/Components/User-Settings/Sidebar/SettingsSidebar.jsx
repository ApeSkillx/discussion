...
function SettingsSidebar() {
  ...
  const [isSidebarVisible, setIsSidebarVisible] = useState(windowWidth > responsive_ss_width);
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setIsSidebarVisible(window.innerWidth > responsive_ss_width);
    };
  ...
  // Components
  const SettingsSidebarItem = ({ link, icon, title }) => {
    const linkTo = link.startsWith('/') ? link.slice(1) : link;
    return (
